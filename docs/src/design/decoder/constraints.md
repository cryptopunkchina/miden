...
* By row $9$ the group is fully executed. This decrements the group count and set op index to $0$.
...
When a `SPAN` or a `RESPAN` operations are executed, we compute the values of the rows to be added to the op group table as follows:
